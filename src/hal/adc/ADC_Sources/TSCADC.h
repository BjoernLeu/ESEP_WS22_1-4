...
enum IdleMode
	: unsigned int {
		FORCE_IDLE = 0, NO_IDLE = 1, SMART_IDLE = 2, SMART_IDLE_WAKEUP = 3
};
...
enum TSMode
	: unsigned int {
		FOUR_WIRE_MODE = 1, FIVE_WIRE_MODE = 2, GENERAL_PURPOSE_MODE = 0
};

enum PenIRQ
	: unsigned int {
		PEN_IRQ_0 = 0, PEN_IRQ_1 = 1
};

enum OperationMode
	: unsigned int {
		SINGLE_ENDED_OPER_MODE = 0, DIFFERENTIAL_OPER_MODE = 1
};
...
enum Wakeup
	: unsigned int {
		WAKEUP_DISABLE = 0, WAKEUO_ENABLE = 1
};

enum HWEvent
	: unsigned int {
		HW_INPUT_EVENT = 1, PEN_TOUCH = 0
};

enum ACBias
	: unsigned int {
		INTERNAL_AC_BIAS = 0, EXTERNAL_AC_BIAS = 1
};
